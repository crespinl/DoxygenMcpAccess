...
def _extract_member_doc(compound_xml_path: Path, member_refid: str) -> dict:
...
def _extract_compound_doc(compound_xml_path: Path) -> dict:
    root = ET.parse(compound_xml_path).getroot()
    ...
    for member in compounddef.iter("memberdef"):
        entry = {
            ...
            "signature": f"{_text_of(member.find('type'))} "
                         f"{_text_of(member.find('name'))}"
                         f"{_text_of(member.find('argsstring'))}".strip(),
        }
        if member.get("kind") == "enum":
...
def get_symbol_doc(name: str) -> dict:
    """
...
def list_class_members(class_name: str) -> dict:
    """
    ...
    if "error" in doc:
        return doc
    return {"class": doc.get("name"), "members": doc.get("members", [])}
